...
public class MarkdownMessage implements Message {
    ...
    public static String getColorText(String color, String colorText , String frontText, String backText) {
        if (!"info".equals(color) && !"comment".equals(color) && !"warning".equals(color)) {
            throw new IllegalArgumentException("color should be in {info, comment, warning}");
        }
        return frontText + "<font color=\"" + color + "\">" + colorText + "</font>" + backText;
    }
    public static String getColorText(String color, String colorText) {
        return getColorText(color, colorText, "", "");
    }
